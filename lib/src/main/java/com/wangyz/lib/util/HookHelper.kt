...
object HookHelper {
    ...
    fun hook(view: View, proxyOnClickListener: ProxyOnClickListener) {
        try {
    ...
    fun resetOnclickListener(view: View, onClickListener: View.OnClickListener?) {
        try {
            //首先进行反射执行View类的getListenerInfo()方法，拿到v的mListenerInfo对象，这个对象就是点击事件的持有者
            val method: Method = View::class.java.getDeclaredMethod("getListenerInfo")
            //由于getListenerInfo()方法并不是public的，所以要加这个代码来保证访问权限
            ...
            field.isAccessible = true
            //设置到"持有者"中
            field.set(mListenerInfo, onClickListener)
        } catch (e: Exception) {
            e.printStackTrace()
        }
    }
    ...
    fun getOnclickListener(view: View): View.OnClickListener? {
        try {
            ...
            return field.get(mListenerInfo) as View.OnClickListener
        } catch (e: Exception) {
            e.printStackTrace()
        }
        return null
